...
struct IndexACORN : Index {
    ...
    // Storage for metadata to ensure it lives as long as the index
    std::shared_ptr<std::vector<int>> metadata_storage;

    void add_metadata(const std::vector<int>& new_metadata) {
        metadata_storage->insert(metadata_storage->end(), new_metadata.begin(), new_metadata.end());
    }
...
//     explicit IndexACORN(); // TODO check this is right
    ...
    // Naive deletion
    void delete_node(idx_t node_id);
