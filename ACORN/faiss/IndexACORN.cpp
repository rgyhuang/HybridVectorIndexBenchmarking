...
IndexACORN::IndexACORN(int d, int M, int gamma, std::vector<int>& metadata, int M_beta, MetricType metric)
        : Index(d, metric),
          metadata_storage(new std::vector<int>(metadata)),
          acorn(M, gamma, *metadata_storage, M_beta),
          own_fields(false),
          storage(nullptr)
...
IndexACORN::IndexACORN(Index* storage, int M, int gamma, std::vector<int>& metadata, int M_beta)
        : Index(storage->d, storage->metric_type),
          metadata_storage(new std::vector<int>(metadata)),
          acorn(M, gamma, *metadata_storage, M_beta), // TOOD acorn needs to keep metadata now
          own_fields(false),
          storage(storage)
          /* reconstruct_from_neighbors(nullptr) */ {}
...
void IndexACORN::delete_node(idx_t node_id) {
    acorn.delete_node(node_id);
}
