    ...
    for (int i = 0; i < levels.size(); i++) {
        if (levels[i] > level) {
            ...
            for (size_t j = begin; j < end; j++) {
                if (neighbors[j] < 0) // mod
                    break;
                printf("%d(%d), ", neighbors[j], (*this->metadata_vec)[neighbors[j]]); // mod
                neighset.insert(neighbors[j]); // mod
            }
            printf("]\n");
            int n_neigh = neighset.size();
...
//#pragma omp parallel for reduction(+: tot_neigh) reduction(+: tot_common) \
  reduction(+: tot_reciprocal) reduction(+: n_node)
    for (int i = 0; i < levels.size(); i++) {
        if (levels[i] > level) {   
            if (((op == EQUAL) && ((*this->metadata_vec)[i] == filter)) || (op == OR && (((*this->metadata_vec)[i] & filter) != 0))) {
            // if (metadata[i] == filter) {
                n_node++;
                size_t begin, end;
                neighbor_range(i, level, &begin, &end);
                std::unordered_set<int> neighset;
                printf("\t\t %d (%d): [", i, (*this->metadata_vec)[i]);

                for (size_t j = begin; j < end; j++) {
                    if (neighbors[j] < 0) // mod
                        break;
                    if (((op == EQUAL) && (((*this->metadata_vec)[neighbors[j]] == filter))) || (op == OR && (((*this->metadata_vec)[neighbors[j]] & filter) != 0))) {
...
void add_link(
    ...
    for (size_t i = begin; i < end; i++) { // HERE WAS THE BUG
        ...
        if (neigh == -1) break;
        auto metadata = (*hnsw.metadata_vec)[neigh];
    ...
    if (level == 0) {
        shrink_neighbor_list(qdis, resultSet, end - begin, hnsw.gamma, src, (*hnsw.metadata_vec)[src], hnsw);
...
void search_neighbors_to_add(
    ...
    while (!candidates.empty()) {
        ...
        for (size_t i = begin; i < end; i++) {
            ...
            auto metadata = (*hnsw.metadata_vec)[nodeId];
...
void greedy_update_nearest(
    ...
    for (;;) {
        ...
        for (size_t i = begin; i < end; i++) {
            ...
            auto metadata = (*hnsw.metadata_vec)[v];
            if (v < 0) {
                break;
            }
...
void ACORN::add_links_starting_from(
    ...
    if (level == 0) {
        ::faiss::shrink_neighbor_list(ptdis, link_targets, M, gamma, pt_id, (*this->metadata_vec)[pt_id], *this);
        // printf("shrunk");
    }
...
void ACORN::delete_node(storage_idx_t node_id) {
    // 1. Remove links TO node_id
    for (size_t i = 0; i < levels.size(); i++) {
        if (i == node_id) continue; 
        if (levels[i] == 0) continue; 

        int node_max_level = levels[i] - 1;
        for (int l = 0; l <= node_max_level; l++) {
            size_t begin, end;
            neighbor_range(i, l, &begin, &end);
            
            for (size_t j = begin; j < end; j++) {
                if (neighbors[j] == node_id) {
                    size_t k = j;
                    while (k + 1 < end && neighbors[k+1] != -1) {
    ...
    // 2. Clear links FROM node_id
    int my_max_level = levels[node_id] - 1;
    for (int l = 0; l <= my_max_level; l++) {
        size_t begin, end;
        neighbor_range(node_id, l, &begin, &end);
        for (size_t j = begin; j < end; j++) {
            neighbors[j] = -1;
        }
    }
